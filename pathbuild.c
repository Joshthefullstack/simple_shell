...
char *build_path(char *token, char *value)
{
	...
	command_len = strlen(value) + strlen(token) + 2;
	...
	memset(command_path, 0, command_len);

	command_path = strcat(command_path, value);
	command_path = strcat(command_path, "/");
	command_path = strcat(command_path, token);
